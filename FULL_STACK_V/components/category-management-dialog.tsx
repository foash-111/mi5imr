...
export function CategoryManagementDialog({ contentTypeId, onCategoriesRefresh }: CategoryManagementDialogProps) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Trigger refresh after a short delay to ensure database operation completes
        ...
        if (response.status === 409) {
          ...
          toast({
            title: errorData.error || "تصنيف موجود بالفعل",
            description: errorData.details || "يوجد تصنيف بنفس الاسم أو المعرف",
            variant: "destructive",
          })
        } else {
